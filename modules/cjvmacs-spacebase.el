...
(defun spacebase/run-development-server ()
  ...
  (spacebase/with-project-directory
   (let ((kill-buffer-query-functions nil)
         ...
         (commands '(("Spacebase runserver" "*Spacebase: runserver*" "uv run python manage.py runserver")
                     ("Spacebase celery" "*Spacebase: celery_worker_0*" "uv run python manage.py celery_worker -n 0 -Q celery")
                     ("Spacebase celery" "*Spacebase: celery_worker_1*" "uv run python manage.py celery_worker -n 1 -Q parents")
                     ("Spacebase tailwind" "*Spacebase: tailwind*" "uv run python manage.py tailwind start"))))
